...
디렉터리가 1대1로 대응한다. 모델 산출물(model_output)은 S130 하위에 두고,
나머지 데이터 디렉터리(01/02/04/05)는 여러 단계가 공유하므로 별도 유지한다.
...
│       └── data/최종 데이터/
│
├── 01_corpus/                         # [S110·S130 산출물] 코퍼스·골드셋
...
│
├── 02_intent/                         # [S160 산출물] 기획 문서·의도 라벨
│   ├── texts/                         # 기획서 원문 TXT
│   ├── extracted/                     # 라인별 추출 JSON
│   ├── intent_labels/                 # ★ 의도 라벨 + 강조 강도
│   │   └── hwanjeolgi_intent.json
│   └── _archive/
│       ├── intent_12label/            # 구 릴리즈 3건 의도 프로필
│       └── raw/                       # 원본 PDF (사내 문서 — 외부 공유 금지)
│
├── 04_reaction/                       # [S140·S150·S170 산출물]
