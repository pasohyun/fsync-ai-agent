...
- 모델: Sonnet 4.6 (예산 안전, 긴 GUIDE×요청수라 Opus는 $49.66 초과 위험). Batch API = 50% 할인.
...
사용:
  python3 src/labeling/label_corpus_batch.py --limit 200    # 스모크(200개) — 비용 투영·멀티라벨 평균 점검
  python3 src/labeling/label_corpus_batch.py                # 전수(108k)
  python3 src/labeling/label_corpus_batch.py --model claude-opus-4-8   # 모델 바꾸려면(예산 확인 필수)
"""
import argparse
import json
import os
...
PER_REQ = 50            # 요청당 댓글 수 (GUIDE 반복 절반 → 비용↓)
MAX_TOKENS = 4000
POLL_SEC = 60
...
GUIDE = """당신은 K-pop 밴드 유튜브 댓글 라벨러입니다. 각 댓글에 분명히 해당하는 라벨만 부여하세요. 대부분의 댓글은 라벨 1개입니다. 두 개 이상은 서로 다른 축이 모두 분명할 때만(예: 연주+비주얼). 의심되면 1개만. 표면 단어가 아니라 실제 의미로 판단합니다.
